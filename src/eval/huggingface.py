...
class HFLM(LM):
    ...
    def loglikelihood_rolling(self, requests: List[Instance]) -> List[float]:
        ...
        for (string,) in tqdm([req.args for req in requests], disable=(self.rank != 0), maxinterval=1000,
                              mininterval=100):
    ...
    def _loglikelihood_tokens(
        ...
        pbar = tqdm(total=len(requests), disable=(disable_tqdm or (self.rank != 0)), maxinterval=1000, mininterval=100)
        for chunk in chunks:
            inps = []
            cont_toks_list = []
            inplens = []
            ...
            for _, context_enc, continuation_enc in chunk:
                ...
                # print(inp)
                # print(torch.sum(inp == 0) / inp.numel())
                # print(inp.shape)
                padding_len_inp = (
            ...
            for (cache_key, _, _), logits, inplen, cont_toks in zip(
                ...
                if getattr(self, "seq_reduction", None) is not None:
                    greedy_tokens = greedy_tokens[:, :int(self.seq_reduction * greedy_tokens.size(1))]
                # mask = logits.abs().sum(-1) != 0
                # mask = ~torch.isnan(logits.abs().sum(-1))

                # greedy_tokens = greedy_tokens[mask].unsqueeze(0)
                cont_toks = torch.tensor(
                    cont_toks, dtype=torch.long, device=self.device
                ).unsqueeze(
                    0
                )
                cont_toks = cont_toks[:, :greedy_tokens.shape[1]]
    ...
    def generate_until(self, requests: List[Instance]) -> List[str]:
        ...
        pbar = tqdm(total=len(requests), disable=(self.rank != 0), maxinterval=1000, mininterval=100)
        if self.batch_size == "auto":
            # using rolling window with maximum context
            print("Passed argument batch_size = auto. Detecting largest batch size")
            batch_size = self._detect_batch_size()
